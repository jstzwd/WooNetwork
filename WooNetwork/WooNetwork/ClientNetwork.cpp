...
namespace Woo {
	namespace Client {
			...
			:m_blockingMode(blockingMode)
		{
			...
			{
				m_serverSocket = socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
				...
				if (connect(m_serverSocket, addr->ai_addr, addr->ai_addrlen)!=0)
				{
			...
			if (ioctlsocket(m_serverSocket, FIONBIO, &m_blockingMode)!=0)
			{
